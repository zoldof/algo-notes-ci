...
求めたい答えは出題により［整数[※](#fn)のみ，整数と剰余，小数］の違いがある
①被除数を除数の桁まで調べる
...
①すだれ算をつかって求める
②変則すだれ算をつかって求める
＊3つの数で変則すだれ算を使う場合について、詳しい解説は[こちらの記事](https://zoldof.github.io/algo-notes/test.html?fname=gcd_and_lcm)でまとめています。
...
- 長方形：横×縦
- 平行四辺形：横×高さ
＊高さで切り取った三角形を反対側にもってくると長方形[※](#fn)になるから
...
＊ひっくり返して並べると平行四辺形になるから
- ひし形：対角線×対角線÷2
＊対角線[※](#fn)で区切られた三角形を対角線軸で反転移動させて合成すると長方形になるから
...
# 脚注
<a id="fn"></a>
- **平成20年告示の学習指導要領**：この要領には、そろばんを使った演算方法や、中学生から学習する内容も混在します。
- **百分率の記述法**：計算過程では分数にする方が分かりやすいです。
- **体積の求め方**：問題文に記述された底面図形の公式に従って求めます。
- **整数**：先頭が0でない0〜9の数字列のことです。
- **点**：この意味での「点」は図形名詞。
注意点や疑問点の「点」はメタ的名詞。
小数点の「点」は記号名詞。
- **この場合の長方形**：正方形を含みます。
- **対角線が直角に交わる理由**：この疑問を確かめるには、中2数学の［二等辺三角形の性質］を学ぶ必要があります。ひとまずの理解として、正方形を対角線上で縦横に引き延ばしても、その図形は対角線で4つ折りにすると頂点が重なることから、疑問点を確かめるのが分かりやすいです。
- **縮小図と縮図の違い**：図面や地図の世界では「縮図」という語が歴史的に根付いており、算数でも「縮図」と呼ぶようになりました。
- **参考サイト**：この記事は、これらサイトの他にも色々な他情報を［収集，取捨選択］して学び直しに最適な形に仕上げました。
