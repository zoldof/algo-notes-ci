...
## 修正履歴
- 内外開き
  - ユーザーの閲覧特性や擬似的なアプリ表示に合わせて、内開きを［メニュー］外開きを［はじめに］として区別できるようにした
    ↑拡張属性を使わなくてもCIページを区別できるため、リンクの記載方法は同じ

- しおり機能の新設
  - 親要素の見出し基準に設定も可能だが、汎用性を持たせるために［ビューポート基準の座標計算のアルゴリズム］を維持することとした
  
- メニューページの移設
  - Index登録にラッパーページのリダイレクトが影響していた模様なので、メニューページのみdist同様に_layoutで対応できるように設定を変更した

- リンクの新規タブ対応
  - markedではrenderer.linkの設定をhtml化の前段階に追加することで全対応させた
  - kramdownではrubyで対応可能だが、既存タブへの互換性を残すため拡張属性を使った

- 試行錯誤：［test］test.html ⇔ ［main］
  - こちらはパス構造ではないので気を付ける
  - 意図はそれぞれ［テスト，ビルド用］
  - テスト表示だとフロントマターは残存する
